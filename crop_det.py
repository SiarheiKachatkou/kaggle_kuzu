...
def main():
    random.seed(0)
    np.random.seed(0)
    dtrainval = mmcv.load(DATASET_PATH/'dtrainval.pkl')
    dtrainval_crop = []

    crops = []
    for sample in tqdm(dtrainval):
        img = mmcv.imread(DATASET_PATH/('train_images/' + sample['filename']))
        ...
        if len(bboxes) == 0:
            for x1, y1, x2, y2 in iter_empty_crops(w, h, SIZE - 64, SIZE):
                ...
                mmcv.imwrite(
                    img_crop,
                    DATASET_PATH/('train_crops/' + filename),
                    auto_mkdir=True)
    ...
    mmcv.dump(
        ...
        DATASET_PATH/'dtrain_crop.pkl')
    mmcv.dump(dtrainval_crop, DATASET_PATH/'dtrainval_crop.pkl')
